...
// node_modules/glob-parent/node_modules/is-extglob/index.js
  ...
  function readdir(directory, settings, callback) {
    settings.fs.readdir(directory, (readdirError, names) => {
      ...
      const tasks = names.map((name2) => {
        const path3 = common.joinPathSegments(directory, name2, settings.pathSegmentSeparator);
        return (done) => {
...
function getPagesInfo({defaultEntries, scanDir, scanFile}) {
...
function getMPAIO(root, options) {
  ...
  Object.keys(pages).map((key) => {
    input[key] = _path2.default.resolve(root, scanFile2Html(pages[key].entry, scanFile, filename));
  });
  return input;
}
...
function mpa(userOptions = {}) {
  ...
  return {
    ...
    config(config) {
      ...
      config.server.open = options.open === "" ? getFirstPage(config.build.rollupOptions.input) : options.open;
    },
    configureServer({middlewares: app}) {
      app.use(_connecthistoryapifallback2.default.call(void 0, {
    ...
    closeBundle() {
      ...
      if (options.filename !== "index.html") {
        _shelljs2.default.ls(resolve(`${dest}/${options.scanDir}/**/*.html`)).forEach((html) => {
          _shelljs2.default.mv(html, html.replace(options.filename, "index.html"));
        });
      }
      _shelljs2.default.rm("-rf", resolve(`${dest}/*.html`));
      _shelljs2.default.mv(resolve(`${dest}/${options.scanDir}/*`), resolve(dest));
